...
public:
    KITTIData(int im_width, int im_height,
        ...
        : im_width_(im_width), im_height_(im_height), fx_(fx), fy_(fy), cx_(cx), cy_(cy), depth_scaling_(depth_scaling), num_class_(num_class)
    {
    ...
    void process_depth_img(const int scan_id, const cv::Mat &depth_img,
                           pcl::PointCloud<pcl::PointXYZL> &cloud, pcl::PointXYZ &origin, bool reproject)
    {
    ...
    void reproject_imgs(const int current_scan_id, seddom::SemanticBKIOctoMap<12, 3> &map)
    {
    ...
    void reproject_img(const int scan_id, const int reproj_id, seddom::SemanticBKIOctoMap<12, 3> &map)
    {
        Eigen::Matrix4f transform = get_current_pose(scan_id);
        for (int32_t i = 0; i < im_width_ * im_height_; ++i)
        {
            ...
            if (pix_depth > 0.1)
            {
                ...
                auto node = map.search(pt.x, pt.y, pt.z);
                if (node != map.end_leaf() && node->get_state() == seddom::State::OCCUPIED)
                {
